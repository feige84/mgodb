...
func (dbs *DBServer) Session() *mg.Session {
	if dbs.server == nil {
		dbs.start()
	}
	if dbs.session == nil {
		mg.ResetStats()
		var err error
		dbs.session, err = mg.Dial(dbs.host + "/test")
		if err != nil {
			panic(err)
		}
	}
...
// checkSessions ensures all mg sessions opened were properly closed.
...
func (dbs *DBServer) checkSessions() {
	...
	panic("There are mg sessions still alive.")
}
